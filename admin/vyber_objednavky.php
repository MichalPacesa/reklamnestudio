...
{ 
	include "navbar.php";
	echo "<p class='oznam'>Nemáte práva na výber objednávky k cenovej ponuke.</p>";exit;
}


$server=$_SERVER['HTTP_HOST'];

if($_SERVER['HTTP_REFERER'] !="http://".$server.'/'.$dir_name.'/'.'admin/vyber_objednavky.php'
AND $_SERVER['HTTP_REFERER'] != "http://".$server.'/'.$dir_name.'/'."admin/vyber_objednavky.php?doprava=1" AND  $_SERVER['HTTP_REFERER'] != "http://".$server.'/'.$dir_name.'/'."admin/vyber_objednavky.php?dolava=1")  
{
	$_SESSION["offset"]=0;
	$_SESSION["stranka"]=1;
	$offset = 0;
...
<h1>Výber objednávky</h1>
<table id="filtre" class = "zoznam" align="center" cellpadding="0" cellspacing="0">
	<tr>
		<td style = "text-align:center !important; width:15%;"><a href="cenova_ponuka.php">Späť na cenovú ponuku</a></td>
		...
		<p><form action="vyber_objednavky.php" method="post">
		Stav Objednávky:
...
if ($offset > $pocet_riadkov-1){ // ak $offset je vacsi ako pocet zaznamov 
	...
	$sql="SELECT * FROM objednavka o LEFT JOIN zamestnanec z ON o.ID_zamestnanca = z.ID_zamestnanca WHERE 1
	$podmienka AND o.ID_objednavky NOT IN (select ID_objednavky as ID_z_cenovej_ponuky from cenova_ponuka) ORDER BY o.ID_objednavky DESC LIMIT $limit OFFSET $offset";
		...
		<tr>
			...
			<div class="container d-flex ms-3">
					...
					<form  class="me-4" action="cenova_ponuka.php" method="post">
						<input class="mt-1" type="image" name="vybrat" Value="Vybrať" src="images/arrow-right-from-bracket-solid.png" title="Vybrať objednávku" style="width:30px;">
						<input type="hidden" name="ID_vybratej_objednavky" value="<?php echo $riadok["ID_objednavky"];?>">
...
<div>
	<table id="sipky" border="1" align="center" cellpadding="0" cellspacing="0">
		<tr><td align="left">
		<a href="vyber_objednavky.php?dolava=1"><img src="images/left-long-solid.png" title="Doľava" style="width:35px; margin-left:4px;"></a>
		</td>
		<td align="center">Stránka: <?php echo $stranka;?></td>
		<td align="right">
		<a href="vyber_objednavky.php?doprava=1"><img src="images/right-long-solid.png" title="Doprava" style="width:35px; margin-right:4px;"></a>
		</td></tr>
	</table>
</div>
