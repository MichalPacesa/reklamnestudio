...
if($ID_pouzivatela)
{
	...
	if($zobrazit) 
	{
		...
		$sql="SELECT * FROM objednavka o LEFT JOIN registrovany_pouzivatel r ON o.ID_pouzivatela=r.ID_pouzivatela WHERE o.ID_objednavky = $ID_objednavky";
	...
	{	
		$sql="SELECT * FROM zakaznik WHERE ID_pouzivatela = $ID_pouzivatela";
		$akcia = "insert"; 
		$nadpis = "Objednávka";
		$vysledok = mysqli_query($dblink,$sql); 
		$riadok = mysqli_fetch_assoc($vysledok);
...
<h2 class="mb-2"><?php echo $nadpis; ?></h2>
</div>
...
<table class="zoznam" border="1" align="center" cellpadding="0" cellspacing="0" style="margin-top:-2rem;">
	<tr><td colspan="2"><b>Kontaktné údaje:</b></td></tr>
	<tr><td>Názov firmy:</td><td style="width:50%;"> <input type="text" name="Obj_Nazov_firmy"  value="<?php echo $Nazov_firmy;?>" <?php echo disabled($akcia);?>></br></td></tr>
	<tr><td>Meno:<span class="hviezdicka">*</span></td><td> <input required type="text" id="Obj_Meno" name="Obj_Meno" value="<?php echo $Meno;?>" <?php echo disabled($akcia);?>></br></td></tr>
	<tr><td>Priezvisko:<span class="hviezdicka">*</span></td><td> <input required  type="text" name="Obj_Priezvisko" value="<?php echo $Priezvisko;?>" <?php echo disabled($akcia);?>></br></td></tr>
	<tr><td>Email:<span class="hviezdicka">*</span></td><td> <input required  type="text" name="Obj_Email" value="<?php echo $Email;?>" <?php echo disabled($akcia);?>></br></td></tr>
	<tr><td>Telefon:<span class="hviezdicka">*</span></td><td> <input required  type="text" name="Obj_Telefon" placeholder="Zadajte v tvare +421..." value="<?php echo $Telefon;?>" <?php echo disabled($akcia);?>></br></td></tr>

	<tr><td colspan="2"><br></td></tr>
	<tr><td colspan="2"><b>Fakturačná adresa:</b></td></tr>
	<tr><td>Ulica a číslo domu (fakturačná adresa):<span class="hviezdicka">*</span></td><td> <input required  type="text" name="Obj_Ulica_cislo_fakturacna" value="<?php echo $Ulica_cislo_fakturacna;?>" <?php echo disabled($akcia);?>></br></td></tr>
	<tr><td>Mesto (fakturačná adresa):<span class="hviezdicka">*</span></td><td> <input required  type="text" name="Obj_Mesto_fakturacna" value="<?php echo $Mesto_fakturacna;?>" <?php echo disabled($akcia);?>></br></td></tr>
	<tr><td>PSČ (fakturačná adresa):<span class="hviezdicka">*</span></td><td> <input required  type="text" name="Obj_PSC_fakturacna" value="<?php echo $PSC_fakturacna;?>" <?php echo disabled($akcia);?>></br></td></tr>
	<tr><td colspan="2"><br></td></tr>
	<tr><td colspan="2"><b>Dodacia adresa:</b> <span class="grey"> (Vyplňte len v prípade, že sa líši od fakturačnej.)</span>  </td></tr>
	<tr><td>Ulica a číslo domu (dodacia adresa):</td><td> <input   type="text" name="Obj_Ulica_cislo_dodacia"  value="<?php echo $Ulica_cislo_dodacia;?>" <?php echo disabled($akcia);?>></br></td></tr>
	<tr><td>Mesto (dodacia adresa):</td><td> <input   type="text" name="Obj_Mesto_dodacia" value="<?php echo $Mesto_dodacia;?>" <?php echo disabled($akcia);?>></br></td></tr>
	<tr><td>PSČ (dodacia adresa):</td><td> <input   type="text" name="Obj_PSC_dodacia" value="<?php echo $PSC_dodacia;?>" <?php echo disabled($akcia);?>></br></td></tr>
	<tr><td colspan="2"><br></td></tr>
	<tr><td colspan="2"><b>Firemné údaje:</b></td></tr>
	<tr><td>IČO:</td><td> <input   type="text" name="Obj_ICO" value="<?php echo $ICO;?>" <?php echo disabled($akcia);?>></br></td></tr>
	<tr><td>DIČ:</td><td> <input   type="text" name="Obj_DIC" value="<?php echo $DIC;?>" <?php echo disabled($akcia);?>></br></td></tr>
	<tr><td>IČ DPH:</td><td> <input   type="text" name="Obj_IC_DPH" value="<?php echo $IC_DPH;?>" <?php echo disabled($akcia);?>></br></td></tr>
	<tr><td colspan="2"><br></td></tr>
		...
		<?php	
				...
				<div class="d-flex align-items-start w-100 border-bottom mb-4 pb-4" id="Sluzba_<?php echo $i;?>">
					...
					<div class="col-md-1 text-center" style="">
						...
						<input type="number" id="Sluzba_<?php echo $i;?>_pocet_kusov" name="Pocet_kusov[<?php echo $i;?>]" value="<?php echo $Pocet_kusov[$i];?>" <?php echo disabled($akcia);?>>
					</div>
					<div class="col-md-2 text-center" id="Farba_<?php echo $i;?>">
						<?php if($Farby[$i] and $Farby[$i]!="" ):
								echo $Select_farieb[$i];
								else: ?>
								<input type="text" id="Sluzba_<?php echo $i;?>_farba" name="Farba[<?php echo $i;?>]" value="<?php echo $Farba[$i];?>" <?php echo disabled($akcia);?>>
						<?php endif?>		
					...
					<div id='Sluzba_<?php echo $i;?>_sirka' class="col-md-1 text-center" style="<?php echo display_style($Zobrazit_rozmery[$i]);?>">
						<input type="text" placeholder="Šírka" name="Rozmer_sirka[<?php echo $i;?>]" value="<?php echo $Rozmer_sirka[$i];?>"<?php echo disabled($akcia);?>>mm 
					</div>
					<div id='Sluzba_<?php echo $i;?>_vyska' class="col-md-1 text-center" style="<?php echo display_style($Zobrazit_rozmery[$i]);?>">
						<input type="text" placeholder="Výška" name="Rozmer_vyska[<?php echo $i;?>]" value="<?php echo $Rozmer_vyska[$i];?>"<?php echo disabled($akcia);?>>mm
					</div>
	...
	<tr><td colspan="2"><br></td></tr>
	<tr><td style = "vertical-align:top !important;">Poznámka:</td><td><textarea type="text" name="Obj_Poznamka" cols="25" rows="5" <?php echo disabled($akcia); ?>><?php echo $Poznamka;?></textarea></td></tr>
				...
				elseif($num_rows!=0):	?>
					<div class="mt-2 mb-2 w-100">
						<?php 
							...
							?>
								<div class="col-md-4 prilohylink" id="Subor_<?php echo $s;?>" style="display:block; float:left;">
									<?php echo '<a href="admin/prilohy/'.$nazov_suboru.'" title="Otvoriť" target="_blank">'.'<div style="float:left">'.$nazov_suboru.'</div></a>';
	...
	<?php if($akcia !='preview'):?>
	<input style="float:left;" type="submit" value="Odoslať" form="form1">
	<?php endif; ?>
	<?php if($akcia == 'preview'):?>
...
<form id="back" action="moje_objednavky.php"></form>
</p>
<?php
